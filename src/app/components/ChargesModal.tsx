...
export default function ChargesModal({
  ...
  const normalizedStatus = (chargeStatus ?? "").toLowerCase().trim() || "none";
    ...
    try {
      ...
      await Promise.all(
        draftCharges.map((c) => {
          ...
          if (!c.id) return createCharge({ orderId, adminId, type: c.type, label: c.label || "Charge", amount, isAdditive: c.is_additive });
          return editCharge({ chargeId: c.id, adminId, label: c.label, amount, isAdditive: c.is_additive });
        })
      );
      ...
      >
        ...
        <div className="flex-shrink-0 mx-5 mt-4">
          <div className="grid grid-cols-3 divide-x divide-[#2A1F14] rounded-xl border border-[#2A1F14] bg-[#0B0704] overflow-hidden">
            <FinStat label="Subtotal" value={`₱${baseTotal.toLocaleString()}`} color="text-white/70" />
            <FinStat
              label="Adjustments"
              value={`${adjustments >= 0 ? "+" : ""}₱${adjustments.toLocaleString()}`}
              color={adjustments >= 0 ? "text-emerald-400" : "text-rose-400"}
            />
            <FinStat label="Final Total" value={`₱${finalTotal.toLocaleString()}`} color="text-[#E8C98A]" />
          </div>
        </div>

        {/* ── CHARGES LIST ── scrollable */}
        <div className="flex-1 overflow-y-auto px-5 pt-4 pb-2 space-y-2 min-h-0">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 rounded-xl border border-dashed border-[#2A1F14]">
              <p className="text-[11px] font-bold text-white/25 uppercase tracking-[0.15em]">No Charges Yet</p>
              {isEditable && (
                <p className="mt-1 text-[10px] text-white/20">Click "Add Charge" below to get started</p>
              )}
            </div>
          ) : (
            draftCharges.map((c, i) => (
              <div
                key={i}
                className="rounded-xl border border-[#2A1F14] bg-[#0B0704] overflow-hidden"
              >
                {isEditable ? (
                  ...
                  <div className="flex items-center justify-between gap-3 px-4 py-3">
                    ...
                    `}>
                      {c.is_additive ? "+" : "−"}₱{Number(c.amount || 0).toLocaleString()}
                    </span>
                  </div>
                )}
              </div>
