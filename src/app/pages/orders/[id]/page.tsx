...
export default function OrderDetailsPage() {
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(`
            ...
            configuration:configuration_id (
              selected_size,
              furniture:furniture_id (id,name,thumbnail_url,model_url),
        ...
        setOrder({
          ...
          configuration: config
            ? { selected_size: config.selected_size, furniture, material, color }
            : null,
        });
      } catch (err) {
  ...
  const { furniture, material, color, selected_size } = order.configuration ?? {};
    ...
    <div className="min-h-screen bg-[#FFF8F0] text-[#4B3F3F] font-sans">
      ...
      <section className="max-w-5xl mx-auto grid md:grid-cols-2 gap-10 px-8 pb-16">
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-2xl font-semibold">{furniture?.name}</h2>
          ...
          <div className="flex gap-3 mt-6">
            {furniture?.model_url && (
              <button
                onClick={() => router.push(`/furniture/${furniture.id}/3d`)}
